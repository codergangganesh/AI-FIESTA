...
export default function HistoryDetailModal({ session, onClose }: HistoryDetailModalProps) {
  ...
  const getUserAvatar = () => {
    ...
    return (
      <div className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${darkMode ? 'bg-blue-600' : 'bg-blue-500'
        }`}>
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className={`relative w-full max-w-6xl max-h-[90vh] overflow-hidden rounded-2xl border shadow-2xl backdrop-blur-sm ${isVisible ? 'animate-scaleIn' : 'scale-95 opacity-0'} transition-all duration-300 ${darkMode
      ...
      >
        ...
        <div className="p-6 h-full flex flex-col">
          ...
          <div className={`mb-6 pb-4 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            ...
            <div className={`rounded-xl p-5 mb-6 border-2 ${darkMode ? 'bg-gradient-to-r from-blue-900/30 to-blue-900/10 border-blue-700/50' : 'bg-gradient-to-r from-blue-50 to-blue-100 border-blue-200'
              }`}>
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <div className={`text-xs px-2 py-1 rounded-full font-semibold ${darkMode ? 'bg-blue-800 text-blue-300' : 'bg-blue-200 text-blue-700'
                      }`}>
            ...
            <div className="flex justify-end">
              <button
                ...
                className={`p-2 rounded-lg transition-all duration-300 transform hover:scale-110 ${copied
                    ? 'text-green-500'
                    : darkMode
          ...
          <div className="flex-1 overflow-hidden">
            ...
            {session.responses && session.responses.length > 0 ? (
                    ...
                    {session.selectedModels.map((modelId: string) => {
                      ...
                      return (
                        <div
                          ...
                          className={`rounded-xl border-2 transition-all duration-200 ${session.bestResponse === modelId
                              ? darkMode
                          ...
                          <div className="p-4 flex flex-col h-full">
                            ...
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center space-x-3">
                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold text-sm ${session.bestResponse === modelId
                                ...
                                <div>
                                  ...
                                  <div className="flex items-center space-x-2">
                                    ...
                                    {session.bestResponse === modelId && (
                                      <div className={`flex items-center space-x-1 text-xs ${darkMode ? 'text-amber-400' : 'text-amber-600'
                                        }`}>
                              ...
                              {response?.responseTime !== undefined && response?.responseTime > 0 && (
                                <div className={`flex items-center space-x-1 text-xs px-2 py-1 rounded-full ${darkMode
                            ...
                            {/* Response Content */}
                            <div className={`rounded-lg p-4 flex-1 flex flex-col ${darkMode ? 'bg-gray-700/30' : 'bg-gray-50'
                              }`}>
                              <div className="flex items-center mb-2">
                                <div className={`text-xs font-semibold px-2 py-1 rounded ${darkMode ? 'bg-purple-900/50 text-purple-300' : 'bg-purple-100 text-purple-700'
                                  }`}>
                                  AI Response
                                </div>
                              </div>
                              {response?.error ? (
                                <div className={`text-sm p-3 rounded flex-1 flex items-center ${darkMode ? 'bg-red-900/30 text-red-300' : 'bg-red-50 text-red-500'
                                  }`}>
                                  <div>{response.error}</div>
                                </div>
                              ) : response?.content ? (
                                <div className="flex-1 flex flex-col">
                                  <div className={`whitespace-pre-wrap text-sm flex-1 overflow-y-auto max-h-[300px] ${darkMode ? 'text-gray-200' : 'text-gray-700'
                                  ...
                                  <div className="mt-2 flex justify-end">
                                    <button
                                      ...
                                      className={`p-1.5 rounded transition-all duration-300 transform hover:scale-110 ${copiedResponses[`${session.id}-${modelId}`]
                                ...
                                <div className={`text-sm italic flex-1 flex items-center ${darkMode ? 'text-gray-400' : 'text-gray-500'
                                  }`}>
                                  No response
                                </div>
                              )}
              ...
              <div className={`text-center py-12 rounded-xl ${darkMode ? 'bg-gray-800/30' : 'bg-gray-50'
                }`}>
