  ...
  const renderStars = () => {
    return [1, 2, 3, 4, 5].map((star) => (
      <button
        ...
        className={`p-1 focus:outline-none ${star <= (hoverRating || formData.rating)
          ? 'text-yellow-400'
          : 'text-gray-300'
          }`}
      ...
      >
        <Star
          className={`w-5 h-5 sm:w-6 sm:h-6 ${star <= (hoverRating || formData.rating)
            ? 'fill-current'
            : 'fill-none'
            }`}
        />
    ...
    <section className="py-16 sm:py-24 relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 sm:mb-16">
          <h2 className={`text-3xl sm:text-4xl md:text-5xl font-bold bg-gradient-to-r bg-clip-text text-transparent mb-4 sm:mb-6 ${darkMode
        ...
        <div className="flex justify-center mb-8">
          <div className={`inline-flex items-center rounded-full p-1 ${darkMode
        ...
        <div className="flex justify-center mb-8 sm:mb-12">
          <div className={`inline-flex p-1 rounded-xl backdrop-blur-sm ${darkMode
            ? 'bg-gray-800/60 border border-gray-700/50'
            : 'bg-white/80 border border-slate-200/50'
            }`}>
            <button
              onClick={() => setActiveTab('pricing')}
              className={`px-4 py-2 sm:px-6 sm:py-3 rounded-xl text-sm sm:text-base font-semibold transition-all duration-300 relative overflow-hidden ${activeTab === 'pricing'
                ? 'text-white'
                : darkMode
                  ? 'text-gray-300 hover:text-white'
                  : 'text-slate-600 hover:text-slate-900'
        ...
        {/* Content Container */}
        <div className={`rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 backdrop-blur-sm transition-all duration-500 relative overflow-hidden ${darkMode
          ...
          {activeTab === 'pricing' && (
            <div>
              <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3">
                {plans.map((plan) => (
                  <div
                    ...
                    className={`relative rounded-2xl p-6 sm:p-8 transition-all duration-300 ${plan.popular
                      ? darkMode
                  ...
                  >
                    ...
                    <div className="text-center mb-6 sm:mb-8">
                      <h3 className={`text-xl sm:text-2xl font-bold mb-2 ${darkMode ? 'text-white' : 'text-slate-900'
                        }`}>
                        {plan.name}
                      </h3>
                      <p className={`text-xs sm:text-sm mb-4 sm:mb-6 ${darkMode ? 'text-gray-400' : 'text-slate-600'
                        }`}>
                        {plan.description}
                      </p>

                      <div className="mb-4 sm:mb-6">
                        <span className={`text-2xl sm:text-3xl md:text-4xl font-bold ${darkMode ? 'text-white' : 'text-slate-900'
                          }`}>
                          {plan.price}
                        </span>
                        <span className={`text-xs sm:text-sm block mt-1 ${darkMode ? 'text-gray-400' : 'text-slate-600'
                          }`}>
                          {plan.period}
                        </span>
                      </div>
                    ...
                    <ul className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
                      {plan.features.map((feature, index) => (
                        <li key={index} className="flex items-start space-x-2 sm:space-x-3">
                          ...
                          <span className={`text-xs sm:text-sm ${darkMode ? 'text-gray-300' : 'text-slate-600'
                            }`}>
                            {feature}
                          </span>
                        </li>
                    ...
                    <button
                      ...
                      className={`w-full py-2 px-4 sm:py-3 sm:px-6 rounded-xl font-semibold transition-all duration-300 relative overflow-hidden group text-sm sm:text-base ${plan.popular
              ...
              {/* Pricing Footer */}
              <div className={`mt-8 sm:mt-12 pt-6 sm:pt-8 border-t text-center ${darkMode ? 'border-gray-700/50' : 'border-slate-200/50'
                }`}>
                <p className={`mb-3 sm:mb-4 text-sm sm:text-base ${darkMode ? 'text-gray-400' : 'text-slate-600'
                  }`}>
