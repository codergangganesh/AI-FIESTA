...
export default function ModernModelShowcase() {
  ...
  return (
    <div className={`py-24 ${darkMode
      ? 'bg-transparent'
      : 'bg-transparent'
      }`}>
      {/* Ambient background elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div className={`absolute top-1/4 left-1/4 w-64 h-64 rounded-full blur-3xl ${darkMode ? 'bg-violet-500/10' : 'bg-blue-400/20'
          }`}></div>
        <div className={`absolute bottom-1/4 right-1/4 w-64 h-64 rounded-full blur-3xl ${darkMode ? 'bg-purple-500/10' : 'bg-purple-400/20'
          }`}></div>
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center mb-20">
          <div className={`inline-flex items-center space-x-2 backdrop-blur-xl rounded-full px-5 py-2.5 mb-6 ${darkMode
            ...
            <Brain className={`w-4 h-4 ${darkMode ? 'text-blue-400' : 'text-blue-500'
              }`} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {featuredModels.map((model, index) => (
            <div
              key={index}
              className={`group backdrop-blur-xl rounded-2xl p-6 transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 relative overflow-hidden cursor-pointer ${darkMode
            ...
            >
              ...
              <div className="relative flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <div className={`w-14 h-14 bg-gradient-to-br ${getProviderColor(model.provider)} rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-lg group-hover:scale-110 transition-transform duration-300 ${darkMode ? 'shadow-violet-500/30' : 'shadow-blue-500/30'
                    }`}>
                    {model.label.charAt(0)}
                  </div>
                  <div>
                    <div className={`font-bold text-lg transition-colors duration-300 ${darkMode ? 'text-white group-hover:text-blue-200' : 'text-slate-900 group-hover:text-blue-600'
                      }`}>
                      {model.label}
                    </div>
                    <div className={`text-sm mt-1 transition-colors duration-300 ${darkMode ? 'text-gray-400 group-hover:text-gray-300' : 'text-slate-600 group-hover:text-slate-500'
                      }`}>
                      {model.provider}
                    </div>
                  </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`text-xs font-semibold transition-colors duration-300 ${darkMode ? 'text-green-400' : 'text-green-600'
                    }`}>
                    Active
                  </span>
                </div>
              ...
              <div className="relative mt-6 flex flex-wrap gap-2">
                {model.capabilities?.slice(0, 3).map((capability, idx) => (
                  <span
                    key={idx}
                    className={`text-xs font-semibold px-2.5 py-1 rounded-full transition-all duration-300 ${darkMode
              ...
              <div className={`relative mt-4 pt-4 border-t transition-all duration-300 ${darkMode ? 'border-gray-700/50 group-hover:border-gray-600/50' : 'border-slate-200/50 group-hover:border-slate-300/50'
                }`}>
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center space-x-2">
                    <Zap className={`w-4 h-4 transition-colors duration-300 ${darkMode ? 'text-gray-400 group-hover:text-blue-400' : 'text-slate-500 group-hover:text-blue-500'
                      }`} />
                    <span className={`transition-colors duration-300 ${darkMode ? 'text-gray-300 group-hover:text-blue-200' : 'text-slate-600 group-hover:text-blue-600'
                      }`}>
                      Speed: Medium
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Star className={`w-4 h-4 transition-colors duration-300 ${darkMode ? 'text-gray-400 group-hover:text-yellow-400' : 'text-slate-500 group-hover:text-yellow-500'
                      }`} />
                    <span className={`transition-colors duration-300 ${darkMode ? 'text-gray-300 group-hover:text-yellow-200' : 'text-slate-600 group-hover:text-yellow-600'
                      }`}>
                      Cost: Medium
                    </span>
                  </div>
        ...
        <div className="text-center mt-12">
          <button
            ...
            className={`inline-flex items-center space-x-2 px-6 py-3 rounded-xl font-semibold hover:shadow-lg transition-all duration-300 hover:scale-105 backdrop-blur-sm ${darkMode
